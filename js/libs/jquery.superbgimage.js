...
(function($) {
	...
	jQuery.fn.superbgCalcSize = function(imgw, imgh) {		
		...
		if (options.nocrop === 1) {
			if ((browserheight / browserwidth) > ratio) {
				...
				newheight = browserheight;//Math.round(browserwidth * ratio);
				newwidth = Math.round(browserheight / ratio);//browserwidth;
			}
		} else if (options.hibridcrop === 1) {
			if (ratio > 0.9) {
				if ((browserheight / browserwidth) > ratio) {
					newheight = browserheight;
					newwidth = Math.round(browserheight / ratio);
	...
	jQuery.fn.superbgimage.defaults = {
		...
		hibridcrop: 0, // crop horizontal images, but not verticals
